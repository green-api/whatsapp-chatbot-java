...
> ВАЖНО: В состоянии бота зарезервирован параметр "scene", в нем хранится актуальная сцена для каждой сессии.
...
`handker` - объект класса, который наследуется от абстрактного класса `BotHandler`. Вы можете переопределить в нем методы,
которые отвечают за обработку вебхуков о состоянии инстанса и девайса `StateInstanceChanged` и `DeviceInfo`.
Если вы не хотите обрабатывать эти типы уведомлений, просто не переопределяйте методы, оставьте класс пустым.
...
абстрактного класса `Scene`. Внутри сцен происходит обработка вебхуков и выполнение вашей бизнес логики.
...
Для этого создайте класс, который наследуется от `Scene` и переопределите метод-обработчик нужного типа вебхука.
